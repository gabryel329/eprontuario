...
<main class="app-content">
    <div class="app-title">
        <div>
            <h1><i class="bi bi-ui-checks"></i> Criar Agenda<span id="displaySelectedProfissional" class="selected-info"></span>
                <span id="displaySelectedData" class="selected-info"></span></h1>
        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" id="profissional_id" name="profissional_id">
                                ...
                                @foreach ($profissional as $item)
                                    <option value="{{ $item->id }}">{{ $item->name }} {{ $item->sobrenome }}</option>
                                @endforeach
                            </select>
                        </div>
                        <button type="button" class="btn btn-primary" onclick="showAdditionalFields()">Confirmar</button>
                    </div>
                        ...
                        <form id="agenda-form" method="POST" action="{{ route('agenda.store') }}" class="form-horizontal">
                            ...
                            <div class="row">
                                <div class="mb-3 col-md-6">
                                    ...
                                    <input class="form-control" id="hora" name="hora" type="time" list="time-options">
                            ...
                            <div class="row">
                                <div class="mb-3 col-md-12">
                                    ...
                                    <input class="form-control" id="consulta_id" name="consulta_id" type="text">
                                </div>
                            </div>
                    ...
                    <tbody>
                        @foreach ($pacientes as $p)
                            <tr>
                                ...
                                <td>{{ $p->nome_social }}</td>
                                <td>
                                    <button class="btn btn-primary" type="button" onclick="selectPaciente('{{ $p->id }}', '{{ $p->name }}', '{{ $p->sobrenome }}')">Selecionar</button>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                response.agendas.forEach(function(agenda) {
                    var row = '<tr>' +
                        '<td>' + agenda.hora + '</td>' +
                        '<td>' + agenda.consulta_id + '</td>' +
                        '<td>' + agenda.status + '</td>' +
                        '<td>' +
                            '<button type="button" class="btn btn-danger" onclick="deleteAgenda(' + agenda.id + ')"><i class="bi bi-x-circle"></i></button>' +
                        '</td>' +
                    '</tr>';
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    fetchAgenda(document.getElementById('selectedData').value, document.getElementById('selectedProfissionalId').value);
