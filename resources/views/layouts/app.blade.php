...
<head>
    ...
    <style>
        .image-container {
            ...
            align-items: center;
            height: 100%;
            /* Ajuste a altura conforme necess√É¬°rio */
        }

        .image-container img {
            max-width: 48%;
            /* Garante que a imagem n√É¬£o ultrapasse a largura da tela */
            max-height: 100%;
            /* Garante que a imagem n√É¬£o ultrapasse a altura da tela */
        }
    </style>
...
<body class="app sidebar-mini">
    ...
    <script>
        ...
        function validarCPF(cpf) {
            cpf = cpf.replace(/[^\d]+/g, ''); // Remove pontos e traÁos
            if (cpf.length !== 11 || /^(\d)\1+$/.test(cpf)) return false; // Verifica se todos os dÌgitos s„o iguais
        ...
        $('#cpf').on('input', function() {
            var cpf = $(this).val();
            if (cpf.length === 14) { // A m·scara usa 14 caracteres (11 dÌgitos + pontos e traÁo)
                if (validarCPF(cpf)) {
