...
@section('content')
                                                ...
                                                <div class="modal-dialog modal-lg">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <form id="responseIA" action="{{ route('ficha') }}"
                                                                method="POST" target="_blank">
                                                                @csrf
                                                                <input type="hidden" name="mae"
                                                                    value="{{ $historico->mae }}">
                                                                <input type="hidden" name="genero"
                                                                    value="{{ $historico->genero }}">
    ...
    <script>
        ...
        function handleResponseIA(event) {
            ...
            const requestBody = `
                Com base nas seguintes informacoes medicas preenchidas, por favor, forneca um diagnostico preliminar:

                Pressao Arterial(PA): ${values.pa}
                ...
                SPO2: ${values.spo2}
                Frequencia Cardiaca(FC): ${values.fc}
                Frequencia Respiratoria(FR): ${values.fr}

                Alem disso, considere os seguintes detalhes relacionados a:
                ...
                Condicao Fisica: ${values.condicao}

                Considerando essas informacoes, qual seria um diagnostico preliminar, recomendacoes para o tratamento ou acompanhamento, remedios e exames a fazer?

                quero que a resposta venha como um texto só sem paragrafo nem item

                comece a frase assim 'Diagnostico Preliminar Com base nas informações médicas fornecidas'
            ...
            // Exibe o modal com a mensagem "Processando..."
document.getElementById('response').innerHTML = 'Processando...';
new bootstrap.Modal(document.getElementById('responseModal')).show();

fetch('{{ route('generateIA.index') }}', {
    method: 'POST',
    headers: {
    ...
    .then(data => {
        let resposta = data.text || 'Resposta não encontrada';

        // Encontra a posição da frase "Diagnóstico Preliminar" e corta a resposta
        const index = resposta.indexOf('Diagnóstico Preliminar: Com base nas informações médicas fornecidas');
        if (index !== -1) {
            resposta = resposta.substring(index); // Mantém apenas a parte a partir da frase
        }
    ...
    .catch(error => {
        ...
        document.getElementById('response').innerHTML = 'Erro ao processar a solicitação.';
    });
